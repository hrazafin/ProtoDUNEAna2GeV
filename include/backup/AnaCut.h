...
class AnaCut
{
  ...
  private:
    ...
    /*
    //2GeV
    const double MCmeanStartX = -30.78; // in cm
    const double MCmeanStartY = 422.3;
    const double MCmeanStartZ = 0.105;
    const double MCmeanThetaX = 101.7 * TMath::DegToRad(); // in rad.
    const double MCmeanThetaY = 101.2 * TMath::DegToRad();
    const double MCmeanThetaZ = 16.45 * TMath::DegToRad();
    const double MCsigmaStartX = 4.303;
    const double MCsigmaStartY = 4.048;
    const double MCsigmaStartZ = 0.207;

    const double DATAmeanStartX = -27.97; // in cm
    const double DATAmeanStartY = 424.7;
    const double DATAmeanStartZ = 4.006;
    const double DATAmeanThetaX = 100.7 * TMath::DegToRad(); // in rad.
    const double DATAmeanThetaY = 103.6 * TMath::DegToRad();
    const double DATAmeanThetaZ = 17.65 * TMath::DegToRad();
    const double DATAsigmaStartX = 4.236;
    const double DATAsigmaStartY = 4.768;
    const double DATAsigmaStartZ = 1.169; //end
    */
    //for 2 GeV with Richie parameter
    /*
    const double MCmeanStartX = -36.25; // in cm
    const double MCmeanStartY = 422.3;
    const double MCmeanStartZ = 30;
    const double MCmeanThetaX = 101.6 * TMath::DegToRad(); // in rad.
    const double MCmeanThetaY = 101.2 * TMath::DegToRad();
    const double MCmeanThetaZ = 16.49 * TMath::DegToRad();
    const double MCsigmaStartX = 5.092;
    const double MCsigmaStartY = 4.775;
    const double MCsigmaStartZ = 0.205;

    const double DATAmeanStartX = -32.53; // in cm
    const double DATAmeanStartY = 424.7;
    const double DATAmeanStartZ = 30;
    const double DATAmeanThetaX = 100.4 * TMath::DegToRad(); // in rad.
    const double DATAmeanThetaY = 103.6 * TMath::DegToRad();
    const double DATAmeanThetaZ = 16.92 * TMath::DegToRad();
    const double DATAsigmaStartX = 4.952;
    const double DATAsigmaStartY = 4.865;
    const double DATAsigmaStartZ = 0.205; //end */
