...
void TemplateFitter::fcn(int &npar, double *gin, double &f, double *par, int iflag){
  ...
  //sfactor += h1->Integral()+h2->Integral()*pow(par[0],2)/(h1->Integral()+h2->Integral()*par[0]);
  ...
  for (int i = i0; i<=i1; ++i){
    ...
    //if(i != i1) chisq += pow(x-y-z*par[0],2)/(sfactor*x+(y+z*par[0]));
    //else chisq += pow(x-y-z*par[0],2)/(x+(y+z*par[0]));
    // compond fit
    //chisq += pow(x-y-z*par[0],2)/(sfactor*x+(y+z*par[0]));
